"""Running the xmen ETL application"""
import argparse
import logging
import logging.config

import yaml

from xmen.common.s3 import S3BucketConnector
from xmen.transformers.xmen_transformer import xmenETL, xmenSourceConfig, xmenTargetConfig


def main():
    """
        entry point to run the xmen ETL job.
    """
    # Parsing YML file
    parser = argparse.ArgumentParser(description='Run the xmen ETL Job.')
    parser.add_argument('config', help='A configuration file in YAML format.')
    args = parser.parse_args()

#    config = 'C:/Daten/xmen_project_old/xmen_1234/configs/xmen_report1_config.yaml'
#    config = yaml.safe_load(open(config))

    config = yaml.safe_load(open(args.config))
    # configure logging
    log_config = config['logging']
    logging.config.dictConfig(log_config)
    # reading s3 configuration
    s3_config = config['s3']
    # creating the S3BucketConnector classes for source and target
    s3_bucket_src = S3BucketConnector(access_key=s3_config['access_key'],
                                      secret_key=s3_config['secret_key'],
                                      endpoint_url=s3_config['src_endpoint_url'],
                                      bucket=s3_config['src_bucket'])
    s3_bucket_trg = S3BucketConnector(access_key=s3_config['access_key'],
                                      secret_key=s3_config['secret_key'],
                                      endpoint_url=s3_config['trg_endpoint_url'],
                                      bucket=s3_config['trg_bucket'])
    # reading source configuration
    source_config = xmenSourceConfig(**config['source'])
    # reading target configuration
    target_config = xmenTargetConfig(**config['target'])
    # reading meta file configuration
    meta_config = config['meta']
    # creating xmenETL class
    logger = logging.getLogger(__name__)
    logger.info('xmen ETL job started.')
    xmen_etl = xmenETL(s3_bucket_src, s3_bucket_trg,
                         meta_config['meta_key'], source_config, target_config)
    # running etl job for xmen report 1
    xmen_etl.etl_report1()
    logger.info('xmen ETL job finished.')


if __name__ == '__main__':
    main()
